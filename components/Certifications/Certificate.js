...
import styles from '../../styles/components/Certifications/Certificate.module.css'

const Certificate = ({ certificate }) => {
  const { name, description, issuingOrganization, expires, expirationDate, issueDate, credentialId, credentialURL } = certificate
  ...
  const fromApiDateToVerboseDate = (initialDate, finalDate, expires) => {
    ...
    return `${months[Number(initMonth) - 1]} ${initYear}${expires ? ` - ${months[Number(finalMonth) - 1]} ${finalYear}` : ''}`
  }
  ...
  return (
    <div className={styles.container} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      ...
      <div className={styles.certificateContainer}>
        <div className={styles.date}><span>{fromApiDateToVerboseDate(issueDate, expirationDate, expires)}</span></div>
        ...
        <section className={styles.content}>
          <article>
            <h4>Description:</h4>
            <p className={styles.description}>{showingMore ? description : `${description.split('.')[0]}.`} <button onClick={handleReadMore}>{showingMore ? 'Show less' : '... Read more'}</button></p>
          </article>
          <article>
            <h4>Credential ID:</h4>
          ...
          <article>
            ...
            <Link href={credentialURL} target="_blank" rel="noopener noreferrer" aria-label={`${name} certificate url`}>{credentialURL}</Link>
          </article>
        </section>
      </div>
    </div>
